...
interface ILoCircle {
  ...
  // returns explosion after collide
  ILoCircle collideExplode(ILoCircle other);

  // helper method of collideExplode
  ILoCircle collideExplodeHelper(Circle that);
...
class MtLoCircle implements ILoCircle {
  ...
  public ILoCircle spawn() {
    ...
    for (int i = 0; i <= numSpawn; i++) {
      ...
      if (direction == 1) {
        ...
        DoublePosn speed = new DoublePosn(-7, 0);
  ...
  // returns explosion after collide
  public ILoCircle collideExplode(ILoCircle other) {
    return other;
  }

  // helper method of collideExplode
  public ILoCircle collideExplodeHelper(Circle that) {
    return this;
  }
...
class ConsLoCircle implements ILoCircle {
  ...
  public ILoCircle spawn() {
    ...
    for (int i = 0; i <= numSpawn; i++) {
      ...
      if (direction == 1) {
        MyPosn position = new MyPosn(509, (r.nextInt(225) + 25));
        DoublePosn speed = new DoublePosn(-7.0, 0.0);
        ships = ships.add(new Ship(position, speed));
      }
      else {
        MyPosn position = new MyPosn(-9, (r.nextInt(225) + 25));
  ...
  // returns explosion after collide
  public ILoCircle collideExplode(ILoCircle other) {
    return this.rest.collideExplode(other.collideExplodeHelper(this.first));
  }

  // helper method of collideExplode
  public ILoCircle collideExplodeHelper(Circle that) {
    if (this.first.touching(that)) {
      ILoCircle bullets = this.rest;
      int tier = ((Bullet) this.first).tier;
      int numBullets = tier + 1;
      double angle = ((Math.PI * 2) / numBullets);
      for (int i = 1; i <= numBullets; i++) {
        bullets = bullets.add(new Bullet(this.first.position,
            new DoublePosn(8 * Math.cos(angle * i), (8 * Math.sin(angle * i))), tier + 1));
      }
      return bullets;
    }
    else {
      return new ConsLoCircle(this.first, this.rest.collideExplodeHelper(that));
    }
  }

  // counts the number of circle collides
  public int countCollision(ILoCircle other) {
    return this.rest.countCollision(other.collideHelper(this.first))
        + other.countCollisionHelper(this.first);
  }
...
class ExamplesCircle {
  ...
  DoublePosn speed = new DoublePosn(0.0, 20.0);
  ...
  boolean testcollideHelper(Tester t) {
    return t.checkExpect(this.outScreenCircles.collideHelper(this.ship),
        new ConsLoCircle(this.outScreenShip, new MtLoCircle()))
  ...
  boolean testcollide(Tester t) {
    return t.checkExpect(this.outScreenCircles.collide(this.circles), this.mtList)
        && t.checkExpect(this.circles.collide(this.outScreenCircles),
            new ConsLoCircle(this.outScreenShip, new MtLoCircle()))
