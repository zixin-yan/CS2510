...
class MyGame extends World {
  ...
  public MyGame onTick() {
    ...
    this.bullets = tempShip.collide(tempBullet);
    ...
    if (this.currentTick % 28 == 0) {
      return new MyGame(this.nBullets, this.shipsDestroyed,
        ...
        this.bullets.moveAll().removeOffScreen(WIDTH, HEIGHT),
        this.currentTick + 1);
    }
    else {
      return new MyGame(this.nBullets, this.shipsDestroyed,
  ...
  public MyGame onKeyEvent(String key) {
    ...
    if (key.equals(" ") && this.nBullets > 0) {
      return new MyGame(this.nBullets - 1, this.shipsDestroyed, this.ships, new ConsLoCircle(
          new Bullet(new MyPosn(250, 300), new MyPosn(0, -10), 1), this.bullets),
          this.currentTick);
    }
    else {
      return this;
